...
Author:  Your team name, Student Names, Student IDs
Date:    Today's Date
"""
...
class SimpleOscillator:
    ...
    def __init__(self, mass=0.5, spring_constant=20.0, damping= 0.5, initial_position=0.08 ):
        ...
        """
        # Validate parameters
        assert mass > 0, "Mass must be possitive"
        assert spring_constant > 0, "constant must be positive"
        assert damping >= 0 , "non-negative"
        # System parameters
        self.mass = mass 
    ...
    def compute_system_info(self):
        ...
        """
        'omega_0' = np.sqrt(self.k/ self.mass) #Natural frequency
        'damping_ratio': self.d/ (2* np.sqrt(self.mass * self.k )) 
